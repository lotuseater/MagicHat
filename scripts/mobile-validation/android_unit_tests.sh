...
if [[ -z "${JAVA_HOME:-}" || ! -x "$JAVA_HOME/bin/java" ]]; then
  echo "[android-unit-tests] ERROR: JAVA_HOME is not configured for JDK 17" >&2
  exit 1
fi
...
if [[ -z "${ANDROID_SDK_ROOT:-}" || ! -d "$ANDROID_SDK_ROOT" ]]; then
  echo "[android-unit-tests] ERROR: ANDROID_SDK_ROOT or ANDROID_HOME must point to a valid Android SDK" >&2
  exit 1
fi
